...
const ghostFragment = `
  ...
  void main() {
    ...
    vec3 color = source * (0.54 + smoothstep(0.08, 0.86, lum) * 0.5) * stripe;
    color += vec3(1.0) * smoothstep(0.74, 1.0, lum) * 0.035;
...
const floorFragment = `
  ...
  void main() {
    ...
    vec3 reflectedColor = blurred;
    vec3 lightColor = reflectedColor * (0.42 + emission * 0.72);
    vec3 color = lightColor * reflectedShape * floorRipple * 0.62;
    color += vec3(1.0) * wetSheen * 0.24;
    color += reflectedColor * 0.1 * reflectedShape;
...
const shots = [
  {
    name: "Wide Hold",
    duration: 9200,
    from: new THREE.Vector3(0.26, 1.42, 3.18),
    cp1: new THREE.Vector3(0.16, 1.44, 3.06),
    cp2: new THREE.Vector3(-0.02, 1.46, 2.9),
    to: new THREE.Vector3(-0.18, 1.48, 2.74),
    targetFrom: new THREE.Vector3(activeX + 0.76, 2.08, surfaceZ),
    targetTo: new THREE.Vector3(activeX + 0.42, 2.02, surfaceZ + 0.02),
    fovFrom: 52,
    fovTo: 49,
    drift: 0.42,
    roll: 0.0018,
  ...
  },
  {
    ...
    targetFrom: new THREE.Vector3(activeX + 0.58, 2.02, surfaceZ + 0.04),
    targetTo: new THREE.Vector3(activeX + 0.18, 1.98, surfaceZ + 0.08),
    fovFrom: 43,
    fovTo: 42,
    drift: 0.72,
    roll: 0.0028,
  ...
  },
  {
    ...
    targetFrom: new THREE.Vector3(activeX + 0.92, 2.08, surfaceZ),
    targetTo: new THREE.Vector3(activeX + 0.52, 2.02, surfaceZ + 0.02),
    fovFrom: 49,
    fovTo: 46,
    drift: 0.72,
    roll: 0.0022,
  ...
  },
  {
    ...
    to: new THREE.Vector3(0.26, 1.42, 3.18),
    targetFrom: new THREE.Vector3(activeX + 0.94, 2.12, surfaceZ),
    targetTo: new THREE.Vector3(activeX + 0.76, 2.08, surfaceZ),
    fovFrom: 48,
    fovTo: 52,
    drift: 0.5,
    roll: 0.0018,
